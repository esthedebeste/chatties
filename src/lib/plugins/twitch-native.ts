...
export const plugin: Plugin = {
	...
	async init() {
		...
		if (!res.ok) throw new Error("Failed to fetch global badges");
		const data = res.data as { badge_sets: BadgeSets };
	...
	async channelId(channel, id) {
		...
		if (!res.ok) throw new Error("Failed to fetch channel badges");
		const data = res.data as { badge_sets: BadgeSets };
	...
	privMsg(msg) {
		...
		for (const badge of msg.badges) {
			const custom = customBadges.get(msg.channel_id);
			if (
				custom != null &&
				...
				badge.name in badgeSets &&
				badge.version! in badgeSets[badge.name].versions
			) {
