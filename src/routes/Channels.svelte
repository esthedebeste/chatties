<script lang="ts">
	...
	const keydown = (event: KeyboardEvent) => {
		...
		if (!event.shiftKey) {
			...
			if (event.code === "ArrowLeft") {
				...
				if (index > 0) $currentChannel = channels[index - 1];
				else $currentChannel = channels[channels.length - 1];
			} else if (event.code === "ArrowRight") {
				const channels = $joinedChannels;
				const index = channels.indexOf($currentChannel);
				if (index < channels.length - 1) $currentChannel = channels[index + 1];
			...
			if (event.ctrlKey && event.shiftKey && event.code === "ArrowLeft") {
				joinedChannels.update(channels => {
					...
					else if (index === 0) channels.push(channels.shift()!);
					return channels;
				});
			} else if (
				event.ctrlKey &&
	...
	$: chWidth = chTester ? parseFloat(getComputedStyle(chTester).width) : 8;
	$: maxChannelLen =
		Math.max(...$joinedChannels.map(channel => channel.length)) + 5;
	$: windowWidth = window.innerWidth;
	$: rows = Math.ceil(
		($joinedChannels.length * maxChannelLen * chWidth) / windowWidth
	);
	$: channelsPerRow = $joinedChannels.length / rows;
	$: if (channelsPerRow <= 1.61803398875) channelsPerRow = 1;
	else channelsPerRow = Math.ceil(channelsPerRow);
</script>

<svelte:window
	on:keydown={keydown}
	on:resize={() => (windowWidth = window.innerWidth)}
/>
